...
const Home = () => {
  ...
  const fetchUserData = () => {
    fetch(`https://skyva-api.vercel.app/auth/registrations/show?id=1`)
      .then((response) => {
        if (!response.ok) {
  ...
  const fetchRecentProducts = async () => {
    try {
      const response = await axios.get(
        "https://skyva-api.vercel.app/products/recently_added?limit=8"
      );
  ...
  const fetchOnOfferProducts = async () => {
    try {
      const response = await axios.get(
        "https://skyva-api.vercel.app/products/on_offer"
      );
